...
int get_old_messages(box_t* box, char* subscriber_pipe_name) {
    ...
    if (box_fd < 0) {
        fprintf(stderr, "failed: Could not open box %s\n", box->box_name);
        return -1;
    }

    if (tfs_rewind_offset(box_fd) < 0) {
        fprintf(stderr, "failed: Could not rewind offset for box %s\n", box->box_name);
        return -1;
    }
    ...
    if (num_bytes < 0) {
        ...
        fprintf(stderr, "failed: Could not read message from box %s\n", box->box_name);
        return -1;
    }
    while (true) {
...
int remove_box_command(char* box_name) {
    ...
    while (box_node != NULL) {
        ...
        free(prev_box_node->box);

        // should've freed the memory of the box
        // and should've used tfs_unlink to remove the box
    }
    if (box_node == NULL) {
        if (pthread_mutex_unlock(&box_list_mutex) == -1){ // unlock the mutex
            WARN("failed to unlock mutex: %s", strerror(errno));
...
int register_subscriber_command(char* client_named_pipe_path, char* box_name) {
    ...
    if (pipe_fd == -1){
        WARN("failed to open pipe: %s", strerror(errno));
        if (pthread_mutex_unlock(&box_list_mutex) == -1){
            return -1;
        }
        return -1;
    ...
    while (true){
        // see until there is subscriber closes the pipe
        ...
        if (ret == -1){
            ...
            // close the pipe
            if (close(pipe_fd) == -1){
                WARN("failed to close pipe: %s", strerror(errno));
                return -1;
            }
            return 0;
        }
...
int read_publisher_pipe_input(int pipe_fd, char* publisher_named_pipe, char* box_name) {
    ...
    if (num_bytes == 0) {
        ...
        fprintf(stderr, "read EOF from pipe, closing...\n");
        return 1;
    } else if (num_bytes == -1) {
...
int read_pipe_input(int pipe_fd){
    ...
    switch (code) {
        ...
        default:{                
            fprintf(stderr, "failed: Invalid command code: %d\n", code);
            return -1;
        }
    }
